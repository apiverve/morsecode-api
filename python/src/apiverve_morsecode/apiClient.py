...
class MorsecodeAPIClientError(Exception):
...
class ValidationError(MorsecodeAPIClientError):
...
class MorsecodeAPIClient:
    ...
    VALIDATION_RULES = {"text": {"type": "string", "required": True}, "audio": {"type": "boolean", "required": False, "default": False}}
    ...
    def __init__(self, api_key: str, secure: bool = True, debug: bool = False):
        """
        Initialize the MorsecodeAPIClient with the API key and optional settings

        :param api_key: Your APIVerve API key
        :param secure: Deprecated. Always set to True
        :param debug: Enable debug logging (default: False)
        :raises MorsecodeAPIClientError: If API key is invalid
        """
    ...
    def _log(self, message: str) -> None:
        ...
        """
        if self.debug:
            print(f"[MorsecodeAPIClient DEBUG] {message}")
    ...
    def execute(self, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:  # Returns MorseCodeData in 'data' field
        """
        Execute the API request

        :param params: API parameters (optional)
        :return: API response as a dictionary
        :raises MorsecodeAPIClientError: If the request fails or returns an error
        ...
        try:
            ...
            raise MorsecodeAPIClientError(f"Request timeout: {str(e)}")
        except requests.exceptions.ConnectionError as e:
            self._log(f"Connection error: {str(e)}")
            raise MorsecodeAPIClientError(f"Connection error: {str(e)}")
        except requests.exceptions.RequestException as e:
            self._log(f"Request exception: {str(e)}")
            raise MorsecodeAPIClientError(f"Request failed: {str(e)}")
